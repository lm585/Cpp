...
int main(int argc, char * argv[])
{
 ...
 for(int i = 0; i < glob_chr.size(); i++)
 {
  for(int j = 0; j < glob_chr[i].seq.size() - 1; j++)
    //output <<  glob_chr[i].name << '\t' << (j+1) <<  '\t' << glob_chr[i].seq[j] << endl;
    if(toupper(glob_chr[i].seq[j]) == 'C' && toupper(glob_chr[i].seq[j+1]) == 'G')
    {
     output <<  glob_chr[i].name << '\t' << (j+1) <<  '\t' << glob_chr[i].seq[j] << glob_chr[i].seq[j+1]
      << endl; //cg repeat region; CG non-repeat region
    }
 }
