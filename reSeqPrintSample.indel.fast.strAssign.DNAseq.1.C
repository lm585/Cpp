...
int main(int argc, char * argv[])
{
 if(argc != 8)
 {
  cerr <<  "better to use negative value for perc cutoff " << endl;
  cerr << argv[0] << "    ref-genome-fasta   output-file-name pileup-file sample-name   coverage_cutoff(1)  individual-base-cover(1)   individual-base-perc(-0.3)\n";
...
//mapStr already capitalized
bool notStartEnd(const string & mapStr, int i)
{
 //^:. ^:A
 if(i - 2 >= 0 && mapStr[i - 2] == '^')
   return false;
 //.$  A$   (A$     .length() = 2    i=0; i+1 = 1
 if(i + 1 < mapStr.length() && mapStr[i + 1] == '$')
   return false;
 return true;
}
...
string pileup2baseProfile( const pileup & pile, const param & par)
{
 ...
 if(mutCover >= par.coverage)
 {
  ...
  for(int i = 0; i < acg.length(); i++)
    ...
    {
     resStr += acg[i];
     resStr = resStr + '(' + int2str(baseCount[acg[i]]) + ')';
    }
 }

 double indelThresh = 0.8;
 if(delCover  >=  par.coverage && numDel / (double) delCover >= indelThresh ) 
 {
  resStr += '*';
 }

 if(insCover >= par.coverage)
 {
  if(numIn > 0)
  {
   ...
   if(numIn  / (double) insCover >= indelThresh 
      && tempInt / (double) numIn >= indelThresh)
