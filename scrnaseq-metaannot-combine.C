...
int main(int argc, char *argv[])
{
 if(argc != 4)
 {
  cerr << argv[0] << " scRNA-seq.cellID-inorder-file(1)     meta-annotation-file(2)         output(3) " << endl;
  return 1;
 }
...
//store all cell meta annot in a map
//read an in-order cell ID from seurat object
//search map; if found (only one in the map), output the line;
//else output "ID	notapplicable	notapplicable	notapplicable..."
//assume both have quote (or no quote)
 map<string, string> cellIDannotMap;
 ...
 while(!input.eof())
 {
  ...
  if(lineFields.size() >= 2)
    cellIDannotMap[lineFields[0] ] = line;
  getline(input, line);
 }
 input.close();
 ...
 while(!input.eof())
 {
  ...
  if(cellIDannotMap.count(str) == 1)
    output << cellIDannotMap[str] << endl;
  else
  {
   output << str ;
   for(int i = 0; i < 20; i++)
